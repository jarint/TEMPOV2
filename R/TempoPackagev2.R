...
Tempo2 <- function(Frame = Frame, site = 0, output = outPath){
  if (site == 0 && exists("frame")){
    ...
    #########################################################
    #subjects recruited by month - boxplot (all sites)
    DateTable = Table1
    ...
    #########################################################
    #Time of Onset to TNK/Control (all Sites)
    onsettoTNK<-vector()
    onsettoCTRL <- vector()
    for (row in 1:nrow(Table1)){
    ...
    #########################################################
    #Time of Onset to Randomization (all sites)
    ...
    #########################################################
    #Time from CT to TNK/CTRL (all sites)
    ...
    #########################################################
    #occlusion location - base data (all sites)
    ICA = 0
    M1 = 0
    M2 = 0
    M3 = 0
    ...
    #########################################################
    #Randomization time to treatment (all sites)
    timeRDMtoTNK<-vector()
    for (row in 1:nrow(Table1)){
  ...
  if (typeof(site) == "character" && exists("frame")){
    ...
    #ALL Special Case
    if (site == "all"){
      ALL <- c(0:18,20,21,24:26,30:38,40:47,60,61,70:85,87:89,91:96)
      for (i in ALL){
        Tempo2(frame,site = i)
      }
    }
